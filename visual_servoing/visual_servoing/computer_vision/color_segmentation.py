...
def detect_red(img, lower1, upper1, lower2, upper2):
    kernel = np.ones((5, 5), np.uint8)
    image = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)

    filt1 = cv2.inRange(image, lower1, upper1)
    filt1 = cv2.dilate(filt1, kernel, iterations=1)

    filt2 = cv2.inRange(image, lower2, upper2)
    filt2 = cv2.dilate(filt2, kernel, iterations=1)
    filt = cv2.bitwise_or(filt1, filt2)
...
def base_detect_red(img):
    return detect_red(img, [0, 200, 200], [5, 255, 255], [170, 200, 200], [180, 255, 255])
