...
class YoloAnnotatorNode(Node):
    def __init__(self) -> None:
        ...
        # Detection event publishers for the state machine
        self.person_pub = self.create_publisher(Bool, "/person_detected", 10)
        self.parking_pub = self.create_publisher(ConeLocationPixel, "/parking_object_px", 10)
    ...
    def on_image(self, msg: Image) -> None:
        ...
        dets = self.results_to_detections(results[0]) if results else []

        # Publish person detection flag (always publish so state machine knows current state)
        person_msg = Bool()
        person_msg.data = any(d.class_name == "person" for d in dets)
        self.person_pub.publish(person_msg)

        # Publish pixel location of the largest detected chair (parking target)
        chairs = [d for d in dets if d.class_name == "chair"]
        if chairs:
            best = max(chairs, key=lambda d: (d.x2 - d.x1) * (d.y2 - d.y1))
            px = ConeLocationPixel()
            px.u = float((best.x1 + best.x2) / 2)
            px.v = float(best.y2)  # bottom of bbox → ground contact point for homography
            self.parking_pub.publish(px)
